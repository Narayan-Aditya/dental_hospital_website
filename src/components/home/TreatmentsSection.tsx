...
export const TreatmentsSection: React.FC = () => {
  ...
  return (
    <section id="treatments" className="py-20 bg-slate-50 dark:bg-slate-950 transition-colors duration-300 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <div className="p-5 sm:p-6 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {treatment.popular && (
                    <span className="text-[10px] font-extrabold uppercase tracking-wider bg-rose-100 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 px-2 py-0.5 rounded-md">
                      Popular
                    </span>
                  )}
                </div>
        ...
        {selectedTreatment && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 shadow-2xl border border-slate-200 dark:border-slate-800">
              <div className="flex items-start justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                <div>
                  <span className="text-xs font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wider">
                    Treatment Clinical Overview
                  </span>
                  <h3 className="font-display font-bold text-2xl text-slate-900 dark:text-white mt-0.5">
                  ...
                  <div className="flex items-center space-x-3 text-xs text-slate-500 dark:text-slate-400 mt-1">
                    ...
                    <span>•</span>
                    <span className="text-teal-700 dark:text-teal-300 font-semibold">Pain Index: {selectedTreatment.painLevel}</span>
                    <span>•</span>
                    <span className="font-bold text-slate-900 dark:text-white">From ₹{selectedTreatment.startingPrice.toLocaleString('en-IN')}</span>
                  </div>
                </div>
                <button
              ...
              {/* Full Description */}
              <div className="py-4 text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
              ...
              <div className="space-y-3 py-2">
                ...
                <div className="space-y-3">
                  {selectedTreatment.procedureSteps.map((step) => (
                    <div key={step.step} className="flex items-start space-x-3 bg-slate-50 dark:bg-slate-800 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5">{step.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Modal Actions */}
              <div className="flex items-center justify-end space-x-3 pt-6 border-t border-slate-100 dark:border-slate-800 mt-4">
                <button
                  onClick={() => setSelectedTreatment(null)}
