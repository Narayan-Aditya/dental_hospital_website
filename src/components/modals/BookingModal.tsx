...
export const BookingModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl max-w-2xl w-full max-h-[92vh] overflow-y-auto shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
        {/* Header */}
        ...
        {step < 5 && (
          <div className="px-6 pt-4 pb-2">
            <div className="flex items-center justify-between text-[11px] font-bold text-slate-400 dark:text-slate-500 mb-2">
              <span className={step >= 1 ? 'text-teal-600 dark:text-teal-400' : ''}>1. Procedure</span>
              <span className={step >= 2 ? 'text-teal-600 dark:text-teal-400' : ''}>2. Doctor</span>
              <span className={step >= 3 ? 'text-teal-600 dark:text-teal-400' : ''}>3. Date & Slot</span>
              <span className={step >= 4 ? 'text-teal-600 dark:text-teal-400' : ''}>4. Patient Info</span>
            </div>
        ...
        <div className="p-5 sm:p-6 space-y-4 flex-1">
                    ...
                    <div
                      ...
                      className={`p-3 rounded-2xl border cursor-pointer transition-all flex items-start justify-between ${
                        selectedTreatment === t.id
                          ? 'border-teal-600 bg-teal-50/80 dark:bg-teal-950/60 ring-2 ring-teal-500/20 shadow-sm'
                          : 'border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 bg-white dark:bg-slate-900'
                      }`}
                    >
                      <div className="space-y-1">
                        <div className="font-bold text-xs text-slate-900 dark:text-white">{t.title}</div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-1">{t.shortDesc}</div>
                        <div className="text-[11px] font-extrabold text-teal-700 dark:text-teal-400">
                      ...
                      <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 mt-1 ${
                        selectedTreatment === t.id ? 'border-teal-600 bg-teal-600 text-white' : 'border-slate-300 dark:border-slate-700'
                      }`}>
                  ...
                  >
                    <div className="flex items-center space-x-3.5">
                      <img
                        src={doc.image}
                        alt={doc.name}
                        className="w-12 h-12 rounded-xl object-cover object-top border border-slate-200 dark:border-slate-700 shrink-0"
                      />
                      <div>
                        <div className="font-display font-bold text-sm text-slate-900 dark:text-white">{doc.name}</div>
                        <div className="text-xs text-teal-700 dark:text-teal-400 font-bold">{doc.role}</div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{doc.availability}</div>
                      </div>
                    </div>
                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 ${
                      selectedDoctor === doc.id ? 'border-teal-600 bg-teal-600 text-white' : 'border-slate-300 dark:border-slate-700'
                    }`}>
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-center transition-all ${
                        selectedSlot === slot.time
                          ...
                          : 'border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-medium'
                      }`}
                    >
                      <div className="text-xs">{slot.time}</div>
                      <div className={`text-[10px] mt-0.5 ${selectedSlot === slot.time ? 'text-teal-100' : 'text-slate-400 dark:text-slate-500'}`}>
                        {slot.period}
                      </div>
                    </button>
                  ))}
            ...
            <div className="space-y-3.5 animate-fadeIn">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                <div className="text-xs">
                  <span className="font-bold text-slate-800 dark:text-slate-200">Attach Previous X-Ray / Report?</span>
                ...
                <button
                  ...
                  className={`px-3 py-1 text-xs font-bold rounded-lg transition-colors ${
                    xRayAttached ? 'bg-teal-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300'
                  }`}
                >
          ...
          {step === 5 && confirmedAppointment && (
            <div className="space-y-4 animate-fadeIn text-center">
              ...
              <div className="bg-slate-50 dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 text-left space-y-3 shadow-inner">
                <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 pb-2">
                  <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Appointment Reference ID:</span>
                  <span className="font-mono font-extrabold text-sm text-teal-700 dark:text-teal-300 bg-teal-50 dark:bg-teal-950/60 px-2 py-0.5 rounded border border-teal-200/60 dark:border-teal-800">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Patient Name:</span>
                    <span className="font-bold text-slate-800 dark:text-slate-200">{confirmedAppointment.patientName}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Phone Number:</span>
                    <span className="font-bold text-slate-800 dark:text-slate-200">{confirmedAppointment.patientPhone}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Treatment:</span>
                    <span className="font-bold text-teal-800 dark:text-teal-300">{currentTreatmentObj?.title}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Consulting Doctor:</span>
                    <span className="font-bold text-slate-800 dark:text-slate-200">{currentDoctorObj?.name}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Date & Time:</span>
                    <span className="font-bold text-slate-800 dark:text-slate-200">{confirmedAppointment.date} at {confirmedAppointment.timeSlot}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Clinic Location:</span>
                    <span className="font-bold text-slate-800 dark:text-slate-200">Sadrauna, Mohan Rd, Lucknow</span>
                  </div>
                </div>
              </div>
        ...
        <div className="p-5 sm:p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between">
          {step > 1 && step < 5 ? (
            <button
              onClick={() => setStep(prev => prev - 1)}
              className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-800 flex items-center space-x-1 transition-all"
            >
          ...
          {step < 5 ? (
            <button
              ...
              className="px-6 py-2.5 bg-gradient-to-r from-teal-600 to-teal-500 hover:from-teal-700 hover:to-teal-600 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-lg shadow-teal-600/25 flex items-center space-x-1.5 transition-all"
            >
            ...
            <button
              onClick={handleClose}
              className="w-full py-3 bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs rounded-xl shadow-lg shadow-teal-600/25 transition-all"
            >
              Done & Close
            </button>
          )}
